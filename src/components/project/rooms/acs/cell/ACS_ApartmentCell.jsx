...
const ACS_ApartmentCell = ({ apartmentId }) => {
  ...
  return (
    <ApartmentCellProvider apartmentId={apartmentId}>
      <div style={{ gridColumn: 2 + maxX - apartment.x_coordinate, gridRow: 2 + maxY - apartment.y_coordinate }}
           onClick={() => handleApartmentSelect(apartment.id, !isSelected)}>
        <div
          className="component d-flex justify-content-center align-items-center flex-column position-relative cell-outline my-grid-cell">
          <div
            className={`btn-outline-cell-select-on w-100 h-100 position-absolute border pe-none z-index-45 ${isSelected
              ? 'border-5'
              : 'border-0'}`}></div>
          <div className="w-100 h-100 p-0 d-grid z-index-50"
               style={{ gridTemplateColumns: '100%', gridTemplateRows: 'auto 1fr auto' }}>
            <div className="">
            ...
            <div className="p-0 h-100 w-100 d-flex flex-row border-top border-bottom">
              ...
              <div
                className="h-100 w-fill-available ps-2px pe-1px text-truncate lh-sm d-flex flex-column justify-content-center align-items-start gap-0"
          ...
          <div
            className="m-0 position-absolute d-flex justify-content-center align-items-center top-0 bottom-0 start-0 end-0 pe-none z-index-55">
            {isSaving && (
            ...
            <div
              className="position-absolute top-50 end-0 w-0 h-0 d-flex justify-content-center align-items-end translate-middle-y">
              <IconLink icon={Eye} href={viewUrl} variant="outline-primary"
                        className=" m-1px rounded-1 show-on-hover-1 pe-auto wh-resize-button" target="_blank"/>
            </div>
          </div>
        </div>
      </div>
