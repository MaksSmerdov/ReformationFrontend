...
const FPSM_StatusTypeSelector = ({ statusType, setStatusType }) => {
  ...
  const statusTypes = [
    { id: 'type1', color: 'white', bgColor: 'c-status-future-color', text: t('status_assignment_controls.plan_work') },
    {
      id: 'type3',
      color: 'white',
      bgColor: 'c-status-future-raport-color',
      text: t('status_assignment_controls.send_to_workers'),
    },
    {
      id: 'type4',
      color: 'white',
      bgColor: 'c-status-future-raport-color',
      text: t('status_assignment_controls.inform_client'),
    },
    { id: 'type2', color: '#002C41', bgColor: 'c-status-past-color', text: t('status_assignment_controls.mark_work') },
    {
      id: 'type5',
      color: '#002C41',
      bgColor: 'c-status-past-raport-color',
      text: t('status_assignment_controls.by_teams'),
    },
    {
      id: 'type6',
      color: '#002C41',
      bgColor: 'c-status-past-raport-color',
      text: t('status_assignment_controls.send_to_client'),
    },
  ];
  ...
  return (
    <div className="d-flex flex-column gap-1px">
      {statusTypes.map(({ id, color, bgColor, text }) => (
        <div
          key={id}
          className={`w-100 p-0 gap-2 rounded-0 border-0 c-status-button ${bgColor}
            text-${color === 'white' ? 'white' : 'black'} d-grid align-items-center
            cursor-pointer ${statusType === id ? 'active' : ''}`}
          style={{ gridTemplateColumns: 'auto 1fr auto', gridTemplateRows: 'auto' }}
          onClick={() => handleStatusTypeClick(id)}
        >
          <Form.Check
            name="statusType"
            id={id}
            className="ps-2 d-flex align-items-center"
            onChange={() => handleStatusTypeClick(id)}
          >
            <Form.Check.Input
              className="m-0"
              type="radio"
              checked={statusType === id}
              onChange={() => handleStatusTypeClick(id)}
            />
          </Form.Check>
...
export default FPSM_StatusTypeSelector;
